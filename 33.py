...
async def cmd_rajaxflame(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        port = int(port_str)
        if not 1 <= port <= 65535:
            await update.message.reply_text("❌ **INVALID PORT (MUST BE 1-65535)**")
            return
        duration = int(duration_str)
        if not 10 <= duration <= 300:
            await update.message.reply_text("❌ **DURATION MUST BE BETWEEN 10 AND 300 SECONDS**")
            return
    except ValueError:
    ...
    if valid_ngrok:
        try:
            ...
            if tunnel_data and 'public_url' in tunnel_data:
                ...
                await attack_msg.edit_text(f"✅ **RAJAXFLAME TUNNEL CREATED:** `{escape(tunnel_url)}`")
            else:
    ...
    attack_target_esc = escape(attack_target)
    target_port_esc = escape(target_port)
    repo_name_esc = escape(repo_name)
    github_token_masked_esc = escape(github_token[:12] + "...")
    until_time_str = until_time.strftime('%H:%M:%S UTC')
...
🎯 Target: `{attack_target_esc}:{target_port_esc}`
⏱ Duration: {duration} seconds
🕒 Ends: {until_time_str}

🔰 **MODE:** {mode}
📁 Repository: `{repo_name_esc}`
🔑 Token: `{github_token_masked_esc}`
...
async def cmd_setgithub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document and update.message.document.file_name.endswith(".txt"):
        ...
        with open(path, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                token = line.strip()
                if token and len(token) > 10 and not any(ord(c) < 32 for c in token):
        ...
        count = 0
        for token in tokens:
            if len(token) > 10 and not any(ord(c) < 32 for c in token):
                save_github_token(user_id, token)
                count += 1
...
async def cmd_setngrok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(text) > 10 and not any(ord(c) < 32 for c in text):
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if status.get("running"):
        ...
        target_esc = escape(status['target'])
        repo_esc = escape(status['repo'])
        github_token_esc = escape(status['github_token'])
        status_text = f"""
